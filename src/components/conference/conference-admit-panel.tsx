...
export function ConferenceAdmitPanel() {
  ...
  return (
    <div className="glass-card rounded-2xl border border-yellow-400/20 bg-yellow-400/5 overflow-hidden shadow-xl">
      <div className="flex items-center justify-between gap-2 px-3 py-2 sm:px-4 sm:py-3 border-b border-yellow-400/15">
        <div className="flex items-center gap-2 min-w-0">
          <Clock className="w-3.5 h-3.5 text-yellow-400 shrink-0" />
          <span className="text-[11px] sm:text-sm font-medium text-yellow-400 truncate">
        ...
        {waitingParticipants.length > 1 && (
          <button
            onClick={admitAll}
            className="text-[10px] sm:text-xs px-2.5 py-1 rounded-lg bg-green-500/20 border border-green-500/30 text-green-400 hover:bg-green-500/30 transition-colors shrink-0"
          >
            Admit All
          </button>
        )}
      ...
      <div className="divide-y divide-white/5 max-h-40 sm:max-h-60 overflow-y-auto">
        <AnimatePresence>
          {waitingParticipants.map((p) => (
            <motion.div
              ...
              className="flex items-center gap-2.5 px-3 py-2 sm:px-4 sm:py-3"
            >
              <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-yellow-400/20 border border-yellow-400/30 flex items-center justify-center shrink-0">
                <span className="text-[11px] sm:text-xs font-semibold text-yellow-400">
                  {p.username.charAt(0).toUpperCase()}
                </span>
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-xs sm:text-sm text-foreground truncate">{p.username}</p>
