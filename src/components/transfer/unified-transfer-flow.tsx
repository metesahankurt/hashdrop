...
export function UnifiedTransferFlow({ onFilesSelected, onModeChange }: UnifiedTransferFlowProps) {
  const { files, setFiles, status, setMode, mode, setCodeExpiry, codeExpiry, addLog, textContent, setTextContent, error } = useWarpStore()
  ...
  const handleRefreshCode = () => {
    const newCode = generateAndSetCode()
    setIsCopied(false)
  ...
  useEffect(() => {
    if (status === 'connecting') setCurrentStep('connecting')
    if (status === 'transferring') setCurrentStep('transferring')
  }, [status])
